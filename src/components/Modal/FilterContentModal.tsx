...
const FilterContentModal: React.FC<{ data: any; toggle: any; onApplyFilters: (filters: any) => void }> = ({ data, toggle, onApplyFilters }) => {
    ...
    const allTags = ["Technology", "Programming", "Web Development", "Design", "AI", "Machine Learning"];
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>, type: string) => {
        ...
        switch (type) {
            case "role":
                setRoleInput(value);
                if (value.trim() !== "") {
                    const filteredSuggestions = allRoles.filter(role => role.toLowerCase().includes(value.toLowerCase()));
            ...
            case "tags":
                setTagsInput(value);
                if (value.trim() !== "") {
                    const filteredSuggestions = allTags.filter(tag => tag.toLowerCase().includes(value.toLowerCase()));
    ...
    const handleRemoveTag = (tag: string, e: React.MouseEvent<HTMLButtonElement>) => {
                    ...
                    <form>
                        <div className="mb-2 pt-2">
                            ...
                            <div className="w-full pt-2">
                                <label htmlFor="tags" className="block text-sm font-medium text-[#07074D]">Tags</label>
                                <div className="border border-[#e0e0e0] rounded-md p-2 flex flex-wrap items-center gap-2 bg-white">
                                    {tags.map((tag, index) => (
                                        <div key={index} className={`py-1 px-3 text-sm rounded-full flex items-center ${tag.color}`}>
                                            <span className="text-sm text-[#07074D]">{tag.label}</span>
                                            <button onClick={(e: React.MouseEvent<HTMLButtonElement>) => handleRemoveTag(tag.label, e)} className="ml-2 text-slate-800 hover:text-slate-600">
                                                <FontAwesomeIcon icon={faTimes} />
                                            </button>
                                        </div>
                                    ))}
                                    <input type="text" name="tags" id="tags" value={tagsInput} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange(e, "tags")} placeholder="Type to add tags" className="flex-grow outline-none text-sm font-medium bg-transparent" />
                                </div>
