            ...
            <div className={`relative flex flex-col h-[250px] m-1 p-2 rounded-3xl ${data?.bg_color}`}>
                ...
                <div className="flex flex-row items-center mt-4 lg:mt-2 p-1">
                    <div className="basis-2/3">
                        <h5 className="text-sm text-slate-700 tracking-tight">{data?.company || data?.username}</h5>
                        <h3 className="text-lg text-slate tracking-tight line-clamp-2 lg:line-clamp-3">{data?.position}</h3>
                    </div>
                    <div className="basis-1/3 flex mx-auto justify-end">
                        <img src="/assets/avatar.png" className="rounded-full" width="50px" height="10px" alt="test"/>
                    </div>
                </div>

                {/* Tags section */}
                <div className="absolute bottom-2 left-2 right-2 flex flex-wrap gap-2 max-w-full overflow-hidden">
                    <span className="bg-gray-50 text-gray-700 px-3 py-1 rounded-full text-xs">{data?.experience}</span>
                    <span className="bg-gray-50 text-gray-700 px-3 py-1 rounded-full text-xs">{data?.duration}</span>
                    <span className="bg-gray-50 text-gray-700 px-3 py-1 rounded-full text-xs">{data?.participation}</span>
                </div>
            </div>
